...
export class Oauth2Controller {
  ...
  async authorize(
    ...
    if (response_type !== 'code')
      throw new ForbiddenException('response_type must be "code"');
    ...
    if (req.user) {
      ...
      return res.redirect(
        `${this.configService.get<string>(
          'AUTH_CLIENT',
        )}?redirect=${this.configService.get<string>(
          'THIS_URI',
  ...
  async token(@Query() qs: Record<string, string>) {
    ...
    if (!client_id || !client_secret || !grant_type || !redirect_uri)
      throw new ForbiddenException('Request is missing required fields');
    ...
    if (grant_type === 'authorization_code') {
      ...
      const accessToken = await this.oauthService.createAccessToken(
        authCode.client,
        authCode.scopes,
        authCode.user,
      );
      ...
      const oldRefreshToken = await this.oauthService.findRefreshToken(
        refresh_token,
      );
      if (!oldRefreshToken)
        throw new ForbiddenException('Invalid refresh_token');

      this.oauthService.revokeAllUsersAccessTokens(oldRefreshToken.user);
      const accessToken = await this.oauthService.createAccessToken(
        oldRefreshToken.client,
  ...
  async revokeToken(@Query() qs: Record<string, string>) {
    ...
    if (token_type === 'access_token')
      return await this.oauthService.revokeAccessToken(token);
    if (token_type === 'refresh_token')
      return await this.oauthService.revokeRefreshToken(token);
